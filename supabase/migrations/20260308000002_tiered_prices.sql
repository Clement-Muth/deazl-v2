-- Tiered price lookup functions
-- Replaces exact store_id lookup with a 3-level cascade:
--   1. exact      — price reported in the user's specific store
--   2. brand_city — average from same brand in same city
--   3. national   — average from same brand nationwide
...
AS $$
  ...
  SELECT product_id, store_id, price, quantity, unit, confidence FROM exact
  UNION ALL
  SELECT product_id, store_id, price, quantity, unit, confidence FROM brand_city
  UNION ALL
  SELECT product_id, store_id, price, quantity, unit, confidence FROM national;
$$;
...
AS $$
  ...
  SELECT ingredient_name, store_id, price, quantity, unit, confidence FROM exact
  UNION ALL
  SELECT ingredient_name, store_id, price, quantity, unit, confidence FROM city_avg
  UNION ALL
  SELECT ingredient_name, store_id, price, quantity, unit, confidence FROM national;
$$;
