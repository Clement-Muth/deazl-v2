...
begin
  ...
  -- Seed preferences from ingredients that are already linked (pre-existing links)
  insert into user_ingredient_preferences (user_id, ingredient_name, product_id)
  select distinct
    p_user_id,
    lower(trim(ri.custom_name)),
    ri.product_id
  ...
  -- Match unlinked ingredients using word_similarity
  -- word_similarity(short, long) measures how well the short string
  -- matches a contiguous part of the longer string — ideal for
  -- ingredient names like "Lait" matching "Lait bio demi-écrémé"
  for v_rec in
    select distinct
      lower(trim(ri.custom_name)) as norm_name,
  ...
  loop
    select p.id,
           p.name,
           word_similarity(v_rec.norm_name, lower(p.name))
    into   v_product_id, v_product_name, v_sim
    from   products p
    where  word_similarity(v_rec.norm_name, lower(p.name)) > 0.5
    order  by word_similarity(v_rec.norm_name, lower(p.name)) desc
    limit  1;
